function [center,cell_size,cell_index,V,C] = PositionLocator(CellRegion_All,ImageInfo)

center=cell(0); V=cell(0); C=cell(0); cell_size=cell(0); cell_index=cell(0);

for i = 1:size(CellRegion_All,2)
    
    center{i}=CellRegion_All{i}(:,2:3);
    cell_size{i}=CellRegion_All{i}(:,1);
    cell_index{i}=CellRegion_All{i}(:,8);
