...
for i = 1:size(TrackImageIndex, 2)
    ...
    CellRegion_All{i} = (reshape(struct2array(CellRegion), [7, size(CellRegion, 1)]))';
    CellNumDetected(i) = size(CellRegion, 1);
...
if min(CellNumDetected) < 8
    LowCellNumFrame = CellNumDetected == min(CellNumDetected);
...
if max(CellNumDetected) > 300
    ManyCellNumFrame = CellNumDetected == min(CellNumDetected);
